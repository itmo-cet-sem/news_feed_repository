...
function function_test_url_with_working_connection(url) {
...
function function_test_url_with_xhttp(url_complete) {
	return new Promise(function(resolve, reject) {
		const url = new URL(url_complete);
		var that = new XMLHttpRequest();
		that.open('GET', url.hostname + url.pathname, false);
		try {
			that.send();
			resolve(false);
		} catch(exception) {
...
function function_catch_unavailable_image(element_with_image) {
	var test1 = function_test_url_with_xhttp(element_with_image.src);
	var test2 = function_test_url_with_working_connection(element_with_image.src);
	Promise.all([test1,test2]).then(array_results => {
		return 0;
	}).catch(function imageFailed(e) {
		element_with_image.classList.add('hidden');
		return 1;
	});
}

function function_delete_all_unavailable_images() {
	var elements = document.querySelectorAll('[data-imagetotest=\'1\']');
	var i; 
	for(i=0;i<elements.length;i++) {
		var p = function_catch_unavailable_image(elements[i]);
		elements[i].dataset.imagetotest='0';
	}
}

function_delete_all_unavailable_images();
/*
